...
def process_reddit_thread(url, api_key):
    ...
    try:
        ...
        # --- ROBUSTNESS FIX ---
        # Try to find a title in a few common tags, falling back to the page's main <title>
        title_tag = soup.find('h1') or soup.find('h2') or soup.find('title')
        title = title_tag.get_text(strip=True) if title_tag else 'No Title Found'
        
        # This approach of stripping boilerplate and getting all text is more reliable
...
def process_generic_url(url, api_key):
    ...
    try:
        ...
        # --- ROBUSTNESS FIX ---
        # Use a safe method to get the title
...
def process_backrooms_wiki(url, api_key):
    ...
    try:
        ...
        # --- ROBUSTNESS FIX ---
        # Safely find the content div and handle cases where it's missing
